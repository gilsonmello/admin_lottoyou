...
class OAuth2AuthMechanism extends OAuthAuthMechanism {
    ...
    public function login($provider, $callbackURL) {
        ...
        array_walk($query, function(&$value, $key, $substitutions) {
                    $value = str_replace(
                            array_keys($substitutions), array_values($substitutions), $value
                    );
                }, array(
            '{STATE}' => null,
    ...
    public function getAccessToken($provider, $requestToken = null, $callbackURL = null) {
        ...
        $response = $this->apiRequest(
                ...
                //$provider->accessTokenURL, null, $query, $provider->accessTokenRequestMethod
                $provider->accessTokenURL, 'email', $query, $provider->accessTokenRequestMethod
        );
