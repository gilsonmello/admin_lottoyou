...
class BalanceWithdrawController extends AppController {
    ...
    public function index($modal = 0) {
        ...
        if(isset($query['dt_inicio']) && !empty($query['dt_inicio'])) {
            ...
            $options['conditions']['BalanceWithdraw.created >='] = $dt_inicio . ' 00:00:00';
        }

        if(isset($query['dt_fim']) && !empty($query['dt_fim'])) {
            $dt_fim = implode('-', array_reverse(explode('/', $query['dt_fim'])));
            $options['conditions']['BalanceWithdraw.created <='] = $dt_fim . ' 23:59:59';
        }
        ...
        $total = $this->BalanceWithdraw->find('first', [
            'joins' => [
                array(
                    'alias' => 'User',
                    'table' => 'users',
            ...
            'fields' => [
                ...
                'SUM(ABS(BalanceWithdraw.value)) AS total'
            ],
