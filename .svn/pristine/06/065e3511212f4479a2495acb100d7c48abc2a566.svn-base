(function($) {
    ...
    p.load = function(module) {
        // INICIALIZA VARIÁVEIS
        var modules = window.materialadmin;

        // VERIFICA SE O MÓDULO JÁ FOI CARREGADO
        $.each(modules, function(name, object) {
            if (name == module) {
                module = object;
            }
        });

        // SE NÃO FOI CARREGADO PROCEDE COM O CARREGAMENTO
        if (typeof module === 'string') {
            $.ajaxSetup({async: false});
            $.getScript(baseUrl + 'js/sge/' + module + '.js');
