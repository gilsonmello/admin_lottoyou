 ...
 */
	public static function comparison($check1, $operator = null, $check2 = null) {
		...
		/*
		 * Caso o valor informado para comparação esteja no formato,
		 * nacional o sistema converte antes de fazer a validação.
		 */
		if (current(explode(' ', $check1)) == 'R$' || !is_numeric($check1)){
			$check1 = currency($check1, true);
		}
		if (current(explode(' ', $check2)) == 'R$' || !is_numeric($check2)){
			$check2 = currency($check2, true);
		}
